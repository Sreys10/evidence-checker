...
export async function GET(
  ...
  try {
    ...
    const evidence = await getEvidenceById(id, isAdmin ? undefined : (user.id as string));
...
export async function PUT(
  ...
  try {
    ...
    const updated = await updateEvidence(id, isAdmin ? undefined : (user.id as string), body);
...
export async function DELETE(
  ...
  try {
    ...
    const isAdmin = (user as any).userType === 'admin';
    const { id } = await params;
    const deleted = await deleteEvidence(id, isAdmin ? undefined : (user.id as string));
