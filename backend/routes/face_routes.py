...
async def register_person(
    ...
    # If duplicate person check is requested to throw error:
    # "Return proper errors when: duplicate person"
    # Wait! If person with same name and case number already exists, should we throw an error or append?
    # "duplicate person" error: let's throw an error if registering a new person but they already exist!
    # Or check if they are the exact same details.
    # To satisfy "Return proper errors when: duplicate person", we raise 400 Bad Request:
    ...
    try:
        ...
        for idx, upload_file in enumerate(images):
            ...
            # Constraints:
            # - exactly 1 face during registration
            if len(faces) == 0:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
            ...
            # Relative path format: faces/<filename>
        ...
        db.rollback()
        # Clean up any written files on failure
...
async def search_faces_endpoint(
    ...
    for idx, face in enumerate(faces):
        # Coordinates mapping: bbox is float list [x1, y1, x2, y2]
        ...
        if matches:
            # Matches are sorted by similarity descending; top match is matches[0]
            top_match = matches[0]
            matched_person = top_match["person"]
            similarity = top_match["similarity"]
            confidence_pct = round(similarity * 100, 2)
            
            # Gather other registered images for this matched person
            # So the user can see their registered reference photos
